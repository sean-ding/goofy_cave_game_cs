...
public static class ChunkManager
{
	...
	public static Chunk GetChunk(int chunkY, int chunkX, int layer)
	{
		...
		if (_loadedChunks.TryGetValue(chunkKey, out var loadedChunk)) return loadedChunk;
		if (_loadingChunks.TryGetValue(chunkKey, out var loadingChunk)) return loadingChunk.Result;
	...
	public static void LoadSurroundingChunks(int chunkY, int chunkX, int layer)
	{
		...
		for (var x = -1; x < 2; x++)
		{
			...
			if (_loadedChunks.ContainsKey(chunkKey)) continue;
			if (_loadingChunks.ContainsKey(chunkKey)) continue;
	...
	private static Chunk LoadChunk(int chunkY, int chunkX, int layer, int seed,
		IDictionary<(int, int, int), Chunk> loadedChunks)
	{
	...
	public static Point ToLocalPosition(int positionY, int positionX)
	{
		...
		if (chunkX < 0) chunkX += CHUNK_WIDTH;
	...
	public static Dictionary<(int, int, int), Chunk> GetLoadedChunks()
	{
		...
		foreach (var chunk in _loadedChunks) loadedChunks.Add(chunk.Key, chunk.Value);
