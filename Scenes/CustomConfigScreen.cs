...
public class CustomConfigScreen : ScreenObject
{
	...
	public CustomConfigScreen()
	{
		...
		_customConfigMenu = new CustomConfigMenu()
			{ Position = new Point(HorCentered(_customConfig, CUSTOMCONFIGMENU_WIDTH), 2) };
		// add config menu
	...
	public class CustomConfigMenu : ControlsConsole
	{
		public CustomConfigMenu() : base(CUSTOMCONFIGMENU_WIDTH, CUSTOMCONFIGMENU_HEIGHT)
		{
			...
			confirmButton.Click += (_, _) =>
			{
				...
				if (int.TryParse(chunkSize.Text, out var i) && int.TryParse(minChunkArea.Text, out var j) &&
				    int.TryParse(enemyCount.Text, out var k))
				{
