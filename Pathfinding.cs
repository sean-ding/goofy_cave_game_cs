...
public static class Pathfinding
{
		...
		int openLimit)
	{
		...
		while (open.Count != 0 && pathIndex < openLimit)
		{
			...
			if (currentNode.Y == endY && currentNode.X == endX) return CreatePath(currentNode, closed.Count);
			for (var y = -1; y < 2; y++)
			for (var x = -1; x < 2; x++)
			{
				var childKey = new Point(currentNode.X + x, currentNode.Y + y);

				if ((y == 0 && x == 0) || closed.ContainsKey(childKey) ||
				    GetTile(childKey.Y, childKey.X, layer).Blocking) continue;
				...
				{
					if (open[childKey].G > childNode.G) continue;
	...
	private static Node GetCheapest(Dictionary<Point, Node> open)
	{
		...
		foreach (var node in open)
		{
			...
			if (cheapest.F > node.Value.F) cheapest = node.Value;
